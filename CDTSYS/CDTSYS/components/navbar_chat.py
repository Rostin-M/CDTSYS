import reflex as rx
from CDTSYS.state.state import State
...
def sidebar(trigger) -> rx.Component:
    return rx.drawer.root(
        ...
        rx.drawer.portal(
            rx.drawer.content(
                rx.vstack(
                    rx.heading("Chats con CDTSYS", color=rx.color("mauve", 11)),
                    rx.divider(),
...
def modal(trigger) -> rx.Component:
    return rx.dialog.root(
        ...
        rx.dialog.content(
            rx.hstack(
                rx.input(
                    placeholder="Dale un nombre a tu chat...",
...
def navbar():
    return rx.box(
        rx.hstack(
            rx.hstack(
                rx.avatar(fallback="CS", variant="solid"),
                rx.heading("CDTSYS Chat"),
                rx.desktop_only(
                    rx.badge(
                    State.current_chat,
            ...
            rx.hstack(
                rx.link(rx.button("Inicio", background_color=rx.color("blue", 5)),href="/",),
